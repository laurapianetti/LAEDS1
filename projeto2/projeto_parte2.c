...
int main() {
	...
	int c = 0; //índice do vetor pm
	int posicao_inicial; //posição do ponto inical
	int posicao_final; //posição do ponto final
	int inclinacao = 0; //variável que vai determinar a forma da pista
	...
	posicao_inicial = pm[0];
	posicao_final = pm[c-1];

	inclinacao = posicao_final - posicao_inicial; //calculando a inclinação a partir do ponto médio da última linha e da primeira

	if(c < 10) printf("Resultado: Formato da pista nao estimado.\n");
	else if(inclinacao > 40) printf("Resultado: Curva a esquerda.\n");
	else if(inclinacao < -40) printf("Resultado: Curva a direita.\n");
	else printf("Resultado: Pista em linha reta.\n");
}